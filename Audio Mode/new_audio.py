...
class bc_LSTM:
	...
	def __init__(self, args):
		...
		self.PATH = "./data/audio_models/{}_weights_{}.hdf5".format(args.modality,self.classification_mode.lower())
	...
	def get_speakers_data(self):
		...
		PATH = 'data/MELD/'
		speaker_vector = {
			'Chandler':[1.0,0,0,0,0,0,0],
			'Joey':[0,1.0,0,0,0,0,0],
			'Monica':[0,0,1.0,0,0,0,0],
			'Phoebe':[0,0,0,1.0,0,0,0],
			'Rachel':[0,0,0,0,1.0,0,0],
			'Ross':[0,0,0,0,0,1.0,0],
		}
		def speakers(dialogue_ids, split='train'):
			df = pd.read_csv(PATH+split+'_sent_emo.csv')
			dialogue_speakers = []
			for vid, utts in dialogue_ids.items():
				local_speaker = []
				for utt in utts:
					speaker = list(df[(df['Dialogue_ID']==int(vid)) & (df['Utterance_ID']==int(utt))]['Speaker'])[0]
					if speaker in speaker_vector.keys():
						local_speaker.append(speaker_vector[speaker])
					else:
						local_speaker.append([0,0,0,0,0,0,1.0])
				for _ in range(self.data.max_utts-len(local_speaker)):
					local_speaker.append([0,0,0,0,0,0,0])
				dialogue_speakers.append(local_speaker)
			return np.array(dialogue_speakers)

		self.train_x = np.concatenate((speakers(self.data.train_dialogue_ids, 'train'), self.train_x), axis=2)
		self.val_x = np.concatenate((speakers(self.data.val_dialogue_ids, 'dev'), self.val_x), axis=2)
		self.test_x = np.concatenate((speakers(self.data.test_dialogue_ids, 'test'), self.test_x), axis=2)
	...
	def train_ensemble(self):
		...
		#  checkpoint = ModelCheckpoint(self.PATH[:-5]+'_'+str(i)+'.hdf5', monitor='val_loss', verbose=1, save_best_only=True, mode='auto')
		model = []
		if self.modality == "audio":
			for _ in range(7):
				model.append(self.get_audio_model())
				model[-1].compile(optimizer='adadelta', loss='categorical_crossentropy', sample_weight_mode='temporal')
		elif self.modality == "text":
			model = self.get_text_model()
			model.compile(optimizer='adadelta', loss='categorical_crossentropy', sample_weight_mode='temporal')
		elif self.modality == "bimodal":
		...
		for i in range(7):
			checkpoint = ModelCheckpoint(self.PATH[:-5]+'_'+str(i)+'.hdf5', monitor='val_loss', verbose=1, save_best_only=True, mode='auto')
			early_stopping = EarlyStopping(monitor='val_loss', patience=10)
			model[i].fit(self.train_x, self.train_y,
		                epochs=self.epochs,
		                batch_size=self.batch_size,
		                sample_weight=self.train_mask,
		                shuffle=True, 
	...
	def test_ensemble(self):
		...
		predictions = []
		for i in range(7):
			model = load_model(self.PATH[:-5]+'_'+str(i)+'.hdf5')
			intermediate_layer_model = Model(input=model.input, output=model.get_layer("utter").output)
			...
			for idx, ID in enumerate(self.test_id):
				test_emb[ID] = intermediate_output_test[idx]
			pickle.dump([train_emb, val_emb, test_emb], open(self.OUTPUT_PATH[:-4]+'_'+str(i)+'.pkl', "wb"))

			predictions.append(model.predict(self.test_x))
			self.calc_test_result(model.predict(self.test_x), self.test_y, self.test_mask)

		dialogue_pred = []
		for dialogue in predictions:
			for utterance in dialogue:
				print(np.array(utterance).shape)
...
if __name__ == "__main__":
	...
	for directory in ["./data/pickles", "./data/models", "./data/audio_models"]:
